
AucAdvancedConfig = {
	["profile.Default"] = {
		["util.appraiser.item.7048.duration"] = 720,
		["util.automagic.uierrormsg"] = 1,
		["util.appraiser.columnsortcurSort"] = 6,
		["util.appraiser.item.15136:1106.model"] = "VendMarkup",
		["util.appraiser.columnsortcurDir"] = -1,
		["matcherlist"] = {
			"Undercut", -- [1]
			"BeanCount", -- [2]
		},
		["util.appraiser.item.7048.model"] = "Purchased",
		["util.appraiser.item.15136:1106.number"] = -1,
		["util.appraiser.item.7048.stack"] = 1,
		["util.appraiser.classic"] = false,
		["configator.left"] = 358.5184987824678,
		["util.appraiser.item.7048.match"] = false,
		["util.mover.anchors"] = {
			"TOPLEFT", -- [1]
			nil, -- [2]
			"TOPLEFT", -- [3]
			126.4198061481633, -- [4]
			-70.43688271540562, -- [5]
		},
		["stat.sales.ignoredsigs"] = {
		},
		["util.appraiser.item.15136:1106.stack"] = 1,
		["util.appraiser.item.7048.number"] = -1,
		["configator.top"] = 651.6666923574729,
	},
}
AucAdvancedData = {
	["AserArbitrageRealms"] = {
		["Trinity"] = "Ashia",
		["Single Player Project"] = "Ashia",
	},
	["Stats"] = {
	},
	["UtilSearchUiData"] = {
		["Global"] = {
			["configator.left"] = 308.518546243212,
			["configator.top"] = 676.6666686271009,
		},
		["Current"] = {
			["snatch.itemsList"] = {
			},
		},
		["Version"] = 1,
		["SavedSearches"] = {
		},
	},
}
